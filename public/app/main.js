...
app.controller('mainCtrl', function ($scope, productService) {
    ...
    productService.getProductCategories(function (response) {
        ...
        angular.forEach($scope.categories, function (category, key) {
            ...
            if (category.parent == 0) {
                if (category.slug != "buy-one-get-one-free")
    ...
    function JSONToCSVConvertor(JSONData, ReportTitle, ShowLabel) {
        ...
        link.setAttribute("class", "btn btn-large waves-effect waves-light");
        link.href = uri;

        //set the visibility hidden so it will not effect on your web-layout
        link.download = fileName + ".csv";

        link.appendChild(document.createTextNode("Download file now"));

        var pa = document.getElementById('hr');
        pa.appendChild(link);
